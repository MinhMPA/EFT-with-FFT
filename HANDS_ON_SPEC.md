...
Ship `handson/pk_lin_fiducial.txt` — a tabulated `P_L(k)` — and
`handson/delta_k_128.npy`. **H2 must not depend on H1 having gone well.** Anyone
behind loads the file and proceeds.
