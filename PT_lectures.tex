...
\large Lecture notes, three lectures of 90 minutes each}
...
\noindent\textbf{Audience and prerequisites.} Advanced undergraduates and beginning graduate students. The assumed background is linear perturbation theory of the primordial universe: the scalar--vector--tensor (SVT) decomposition, the idea of gauge choices, and the evolution of scalar perturbations through horizon crossing. Statistics of random fields and field-theory methods are developed as needed.

\medskip
\noindent\textbf{Goal.} By the end you should be able to (i) describe the growth of matter perturbations with Eulerian perturbation theory (EPT, also called standard perturbation theory, SPT) and with Lagrangian perturbation theory at first and second order (1LPT and 2LPT), and (ii) derive the anatomy of the one-loop effective-field-theory (EFT) prediction for the galaxy power spectrum in redshift space, evaluate it with a supplied code, and follow how it is brought into contact with survey measurements. This is one standard Eulerian organization of the model behind current full-shape analyses; Lagrangian formulations and other nuisance bases are in active use alongside it.

\medskip
\noindent\textbf{How to use these notes.} They are the \emph{pre-read}, not a transcript. Everything needed to follow the three lectures is here, in more detail than any lecture can carry, so that lecture time can be spent on the argument and one or two derivations. Each lecture opens with a plan showing what will actually be done at the board; the rest is here to consult before and after. Appendices are not lectured. A hands-on session follows Lecture~1 and draws on it directly: the linear spectrum of Section~\ref{sec:linearpk} and the displacement maps of Sections~\ref{sec:lpt}--\ref{sec:2lpt}.

\medskip
\noindent\textbf{Scope.} Assumed throughout: Gaussian adiabatic initial conditions; a single pressureless clustering fluid with scale-independent linear growth; EdS-shaped kernels with exact linear growth; no primordial non-Gaussianity; no multi-fluid relative operators; the plane-parallel approximation for redshift space; no selection effects; one tracer at one effective redshift. The survey measurement operator is only sketched, in Appendix~\ref{app:window}.

\medskip
\noindent\textbf{Conventions.} Conformal time $\tau$, $\dd t = a\,\dd\tau$; comoving coordinates $\vx$; $\cH \equiv \dd\ln a/\dd\tau = aH$; overdots denote $\partial/\partial\tau$. Fourier transforms follow
\begin{equation*}
...
\toprule
$\tau,\ a,\ \cH$ & conformal time, scale factor, $\cH=aH$
...
\medskip
\noindent\textbf{Vocabulary.} Words used below without ceremony, in case any are new.
...
\noindent\textbf{Plan (65 min at the board, 25 for questions).} From the SVT decomposition to the linearized Newtonian equations (0--10) $\rightarrow$ the pressureless fluid, the single-stream closure, and linear growth (10--22) $\rightarrow$ Gaussian random fields and what a power spectrum is (22--32) $\rightarrow$ the transfer function and the linear matter power spectrum (32--40) $\rightarrow$ the displacement map, the Zel'dovich approximation, and shell crossing (40--55) $\rightarrow$ 2LPT and why initial conditions need it (55--65). \emph{Read, not lectured:} the Einstein constraints \eqref{eq:energy_constraint}--\eqref{eq:anisotropic_constraint}, the geodesic derivation (Exercise~\ref{ex:geodesic}), and the Vlasov hierarchy --- state the single-stream closure and move on. This lecture is deliberately the lightest of the three in written content, because it is the most derivation-heavy at the board: linearizing the fluid equations and expanding the Jacobian both take longer to do than to read. It is also the lecture the hands-on session builds on, so it is the one worth running over into the question time rather than rushing. Sources:~\cite{MaBertschinger1995,Bernardeau2002}, \cite[Apps.~A and E]{Jeong2010}.

\subsection{From scalar perturbations to the Newtonian regime}
\label{sec:bridge}

In the previous lectures you decomposed metric perturbations into scalars, vectors, and tensors. For the growth of structure only the scalars matter: vector modes decay in an expanding universe, and tensor modes do not couple to density at linear order. We therefore keep only scalar perturbations and fix the gauge completely. In the conformal Newtonian (longitudinal) gauge the metric reads~\cite{MaBertschinger1995}
\begin{equation}
\dd s^2 = a^2(\tau)\left[-(1+2\psi)\,\dd\tau^2 + (1-2\phi)\,\dd\vx^2\right],
\label{eq:newtonian_gauge}
\end{equation}
with no residual gauge freedom. The potential $\psi$ acts as the Newtonian gravitational potential in the appropriate limit; $\phi$ perturbs the local scale factor.

Matter enters through the perturbed stress-energy tensor. For each species, define the density contrast $\delta \equiv \delta\rho/\bar\rho$, the peculiar velocity $\vu \equiv \dd\vx/\dd\tau$ with divergence $\theta \equiv \nabla\cdot\vu$ (in Fourier space $\theta = i\vk\cdot\vu$), and the scalar anisotropic stress $\sigma$. Two of the linearized Einstein equations are constraints we will need \cite[Eqs.~(23a,d)]{MaBertschinger1995}:
\begin{align}
k^2\phi + 3\cH\left(\dot\phi + \cH\psi\right) &= -4\pi G a^2 \bar\rho\,\delta ,
\label{eq:energy_constraint}\\
k^2\left(\phi - \psi\right) &= 12\pi G a^2 (\bar\rho + \bar P)\,\sigma .
\label{eq:anisotropic_constraint}
\end{align}
Equation~\eqref{eq:anisotropic_constraint} says that $\phi = \psi$ whenever the total anisotropic stress vanishes. Free-streaming neutrinos violate this at the $15\%$ level during radiation domination, but their contribution falls away as matter comes to dominate; for the CDM+baryon system in the matter era, $\phi = \psi \equiv \Phi$ to good accuracy, and we write $\Phi$ from now on.

Cold dark matter is pressureless and stress-free, so energy-momentum conservation gives the exact linear equations \cite[Eq.~(43)]{MaBertschinger1995}
\begin{equation}
\dot\delta_c = -\theta_c + 3\dot\phi,
\qquad
\dot\theta_c = -\cH\,\theta_c + k^2\psi .
\label{eq:cdm_exact}
\end{equation}
These hold on all scales. The Newtonian regime is the sub-horizon limit $k \gg \cH$: in \eqref{eq:energy_constraint} the terms carrying $\cH$ are suppressed by $(\cH/k)^2$, leaving the Poisson equation, and in \eqref{eq:cdm_exact} the dilation term $3\dot\phi$ becomes negligible (the potential is constant for sub-horizon growing modes in the matter era). The system closes into the Newtonian fluid equations in an expanding background,
\begin{equation}
\dot\delta = -\theta,
\qquad
...
\end{equation}
where we used $4\pi G a^2\bar\rho = \tfrac{3}{2}\Om\cH^2$ and, in Fourier space, $\nabla^2 \to -k^2$.

\begin{remark}
Whether perturbations ``grow outside the horizon'' is a gauge-dependent statement: the same mode grows as $(k\tau)^2$ in synchronous gauge but is frozen in the Newtonian gauge until horizon entry \cite[\S 7]{MaBertschinger1995}. Sub-horizon the differences between gauges are suppressed by powers of $\cH/k$ in the variables we care about, which is why the Newtonian treatment below needs no gauge caveats --- suppressed, not identically zero. Gauge awareness from your earlier lectures pays off here.
\end{remark}

\subsection{The Vlasov equation and the pressureless fluid}
\label{sec:vlasov}

We now set up the nonlinear problem in the Newtonian regime, following \cite[\S 2]{Bernardeau2002}. For a nonrelativistic particle in the metric \eqref{eq:newtonian_gauge}, the sub-horizon geodesic equation reduces to
\begin{equation}
\ddot\vx + \cH\,\dot\vx = -\nabla\Phi
\label{eq:newtonian_eom}
\end{equation}
(Exercise~\ref{ex:geodesic}), with $\Phi$ the peculiar potential of \eqref{eq:newtonian_linear}. Collisionless matter is then described by its phase-space density $f(\vx,\vp,\tau)$, with $\vp = am\vu$; phase-space conservation gives the Vlasov equation,
\begin{equation}
