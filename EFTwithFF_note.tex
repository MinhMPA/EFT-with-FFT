...
The Lyman-\(\alpha\) forest case does not require a new FFTLog philosophy. It requires a larger LOS operator basis. The scalar master integral of \cref{eq:master_int} remains the basic analytic building block, and the Schwinger-derived coefficient formula \cref{eq:los_master_coeff_note} remains the efficient way to generate the LOS coefficient functions. What changes is only the highest power of \((\vnhat\cdot \vq)\) that must be retained.

For the Gaussian redshift-space matter and galaxy case, the hierarchy stops at the CLASS-PT family \((A_1,\ldots,C_4)\), or equivalently at \(n\leq 4\) in \cref{eq:los_master_galaxy_families_note}. In the Lyman-\(\alpha\) case, extra LOS structures enlarge the basis and force one to continue the same logic to \(n=5,\ldots,8\), namely to the family \((A_5,\ldots,E_8)\) summarized in \cref{eq:los_master_lya_families_note}.

This is the most useful way to compare the galaxy and Lyman-\(\alpha\) calculations. The Lyman-\(\alpha\) formulas are not a separate class of loop integrals. They are the same master-integral hierarchy pushed to a higher truncation order because the observable contains more LOS structure. In the notebook suite, this is made explicit by keeping the CLASS-PT-style galaxy notebook at the \(A_1,\ldots,C_4\) level and the dedicated Lyman-\(\alpha\) notebook at the \(A_5,\ldots,E_8\) level, while deriving both from the same scalar and Schwinger-generated ingredients.
