...
The real-space matter case is the cleanest entry point because all loop terms may be reduced directly to the scalar master integral \(I(\alpha,\beta)\) of \cref{eq:master_int}. However, once one moves to redshift space, the kernels contain explicit contractions with the LOS direction \(\vnhat\), and it becomes useful to organize the calculation in a larger master-integral hierarchy.
...
\subsection{Biased tracers in redshift space}

Once the LOS structure is understood in the matter-only case, the next step is to add biased tracers. Conceptually, this is simpler than learning redshift space and bias at the same time: one now keeps the redshift-space machinery fixed and only enlarges the operator basis.

\subsubsection{From redshift-space matter to biased tracers}
...
\end{align}
This provides the general bridge from redshift-space matter to biased tracers. The FFTLog strategy itself does not change; what changes is the kernel content and the number of perturbative sectors.
...
The reorganization from real-space matter to biased tracers in redshift space therefore proceeds in four stages.
...
\subsection{Lyman-\(\alpha\) as a higher-order LOS extension}
