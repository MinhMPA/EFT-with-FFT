...
- **Notebooks must run unmodified in plain Jupyter.** No Colab magics except inside `try: from google.colab import files / except ImportError:` guards. **No package installation in the six graded steps** — their only dependencies are numpy and matplotlib. The single exception is Task 9's optional closing section, which may install CAMB, must guard the import, and must fall back to a shipped table when the install or the import fails.
...
handson/
  ...
  T_camb_fiducial.txt                  # generated by Task 9; CAMB-comparison fallback
...
## Task 8: P_L(k) across cosmologies (extends spec step 5)

**Files:**
- Modify: `handson/make_notebooks.py` — insert one `C` cell and one `M` cell **immediately before** the existing variation-table cell (the one printing `variant / k_eq / turnover / rms delta`), and make that table cell reuse the new `VARIANTS` list.

**Interfaces:**
- Consumes: `ns`, `Om`, `h`, `Tcmb`, `T_full`, `make_pk_lin` (all already in scope).
- Produces: `VARIANTS` — a list of `(label, kwargs, ns_value)` triples, consumed by the existing table cell.

**Why:** step 5 currently gives students a five-row table of `k_eq`, turnover and `rms δ`, but never plots `P_L(k)` itself. "The turnover slides with Ω_m" and "the spectrum pivots with n_s" are visual facts delivered as three columns of numbers. This adds the picture, and the table stays as corroboration.

- [ ] **Step 1: Insert the overlay cell before the existing table cell**
...
**Placement matters.** This pair goes *before* the existing `print(f"{'variant':12s} ...")` cell, so students see the picture and then read the numbers off it.

- [ ] **Step 2: Make the existing table cell reuse `VARIANTS`**

In the existing variation-table cell, replace the inline list literal in the `for` statement:
...
Change nothing else in that cell. The five rows and their values must be unchanged.
...
Expected: `solutions execute clean`, and the variation table still printing exactly:
...
If any row changed, the `VARIANTS` refactor altered behaviour — revert and investigate rather than accepting new numbers.

- [ ] **Step 4: Commit**

```bash
git add handson/
git commit -m "Hands-on 1: plot P_L(k) across cosmologies, not just tabulate it

The turnover sliding with Om and the spectrum pivoting with ns were being
delivered as three columns of numbers. The ratio panel also makes the sigma_8
pinning visible, which the table cannot show."
```

---

## Task 9: Compare against a Boltzmann code (optional closing cell)

**Files:**
- Modify: `handson/make_notebooks.py` — append cells at the end, after the closing "What you built" markdown.
- Create: `handson/T_camb_fiducial.txt` (generated in Step 1).

**Interfaces:**
- Consumes: `ns`, `T_nowiggle`, `T_full`, `pk_lin`, `pk_nw` (all in scope).
- Produces: nothing later tasks depend on. This is the notebook's last section.

**Why:** students have just coded a fitting formula they were told reproduces a Boltzmann code, and have taken that on faith. This checks it. The measured answer is a good one: agreement is better than 1% across most of the range, and the worst deviation — 2.67% at `k = 0.0925 h/Mpc` — sits on a BAO peak. The formula is weakest exactly where the physics is hardest.

**Design, per the repo owner's decision:** install CAMB live and compute from it, with a shipped table as the safety fallback. This mirrors the H1→H2 fallback already in the notebook: try the real thing, fall back to a file. The install is the ONLY `!pip install` in the notebook and it lives in this optional closing section — the six graded steps stay install-free.

- [ ] **Step 1: Generate the fallback table**

Do this first; the cell reads it when CAMB is unavailable.

```bash
python3 - <<'EOF'
import numpy as np, camb
Om, Ob, h, ns = 0.31, 0.048, 0.676, 0.965
...
kh, z, pk = camb.get_results(pars).get_matter_power_spectrum(
    minkh=1e-4, maxkh=30.0, npoints=1024)
T = np.sqrt(pk[0]/kh**ns); T /= T[0]          # normalise so T -> 1 as k -> 0
np.savetxt("handson/T_camb_fiducial.txt", np.column_stack([kh, T]),
           header="CAMB linear transfer function at z=0, normalised to T->1 as k->0\n"
                  "Om=0.31 Ob=0.048 h=0.676 ns=0.965, no massive neutrinos\n"
                  "k [h/Mpc]    T(k)", fmt="%.8e")
print("wrote handson/T_camb_fiducial.txt")
EOF
wc -l handson/T_camb_fiducial.txt
```

Expected: 1027 lines (1024 data + 3 header).

- [ ] **Step 2: Append the comparison cells**
...
- [ ] **Step 3: Regenerate, execute, and test the fallback path**

```bash
python3 handson/make_notebooks.py && cd handson && \
jupyter nbconvert --to notebook --execute --stdout \
  H1_gaussian_field_solutions.ipynb 2>&1 | grep -E "camb|deviation|reference" ; cd ..
rm -f handson/delta_k_128.npz
```

Expected: `max deviation over the whole range : 2.67%  at k = 0.0925` and `max deviation over your grid's k band: 2.67%`.

Then prove the fallback works, since the machine running this already has CAMB:

```bash
cd handson && python3 -c "
import json, numpy as np
import matplotlib; matplotlib.use('Agg')
nb = json.load(open('H1_gaussian_field_solutions.ipynb'))
ns_ = {'camb': None}                       # force the fallback branch
for c in nb['cells']:
    if c['cell_type'] != 'code': continue
    s = ''.join(c['source'])
    if 'pip' in s or 'import camb' in s or 'savez' in s or 'colab' in s: continue
    exec(s, ns_)
print('fallback path OK')
"; cd ..
```

Expected: the run prints `reference from the shipped CAMB table` and `fallback path OK`, with the same 2.67% figure. If the deviation differs from the CAMB-computed one by more than 0.01 percentage points, the shipped table does not match what CAMB produces and must be regenerated.

- [ ] **Step 4: Commit**

```bash
git add handson/
git commit -m "Hands-on 1: optional comparison against CAMB

Installs camb and computes T(k) from it, falling back to a shipped table --
the same try-the-real-thing-then-fall-back pattern as the H1->H2 handoff.
EH tracks CAMB to under 1% except near the BAO, where it reaches 2.67% at
k = 0.09: the fit is weakest exactly where the physics is hardest."
```
