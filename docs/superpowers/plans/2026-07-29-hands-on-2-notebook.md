...
**Unmeasured numbers.** Every checkpoint in Tasks 3–8 is a measured value from this session. **Task 9's are not** — FlowPM is not installed here, so the agreement between our `δ₂` and `flowpm.tfpm.lpt2_source` is genuinely unknown. That task is written to measure first and set tolerances second, and flagged as such. This is the one place the plan cannot anchor itself, and the implementer must not invent a tolerance.
